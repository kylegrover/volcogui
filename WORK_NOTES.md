...
- **Lifecycle follow-up:** verify child/grandchild handling after cancellation on a long-running engine process; the real close path passed. Forced terminal exit cannot run normal in-app cleanup. Fix confirmed shutdown issues before step 3. The separate engine error with no output still leaves Run Log disabled.
